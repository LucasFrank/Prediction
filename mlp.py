...
for t in range(MIN_T, MAX_T + 1, STEP_T):
	...
	with open(pathRBM + 'RBM_{}.csv'.format(t), 'w', 1) as rbm_file:
		with open(pathNN + 'NN_{}.csv'.format(t), 'w', 1) as nn_file:
			# Reading CSV
			...
			for p in range(MIN_P, MAX_P + 1, STEP_P):
				for q in range(MIN_Q, MAX_Q + 1, STEP_Q):
					...
					for n in range(MIN_N, MAX_N + 1, STEP_N):
						...
						# Using 'cf' as params to use a custom activation function
						MLP1 = Pipeline(steps=[('rbm', BernoulliRBM(verbose=False, n_components=n)),
								       ('mlp', MLPRegressor(hidden_layer_sizes=(n, n, n,), activation='cf'))])
